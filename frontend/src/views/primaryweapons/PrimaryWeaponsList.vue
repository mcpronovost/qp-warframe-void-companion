...
const doWeaponsList = async (page: string|null) => {
    ...
    // ===---
    try {
        let f = await fetch(`${API}/primary-weapons/${query}`, {
            method: "GET",
...
<template>
  <div v-if="!isLoading && !hasError" class="qp-container">
    <qp-header :title="$t('PrimaryWeapons')" page-type="weapons" />
    ...
    <el-row v-if="listWeapons.length" class="qp-weapons-list">
      <TransitionGroup name="list" mode="out-in">
        <el-col v-for="(weapon, n) in listWeapons" :key="`weapons-${n}`" :span="12" :sm="8" :md="6" :lg="4">
          <div class="qp-weapons-item" @click="$router.push({name:'PrimaryWeaponsDetail',params:{pk:weapon.id,slug:qpslug(weapon.name)}})">
    ...
    <el-row v-else>
      <el-col>
        <div class="qp-weapons-allcomplete">
          ...
          <p v-text="$t('YoucompletedallPrimePrimaryWeapons')"></p>
        </div>
      </el-col>
    </el-row>
  </div>
