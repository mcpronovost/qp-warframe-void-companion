...
const doWeaponDetail = async (update = false) => {
    ...
    // ===---
    try {
        let f = await fetch(`${API}/secondary-weapons/${route.params.pk}/`, {
            method: "GET",
...
const doWeaponOwn = async (id: Number, doall: Boolean = false) => {
  ...
  // ===---
  try {
      let f = await fetch(`${API}/me/secondary-weapons/components/create/`, {
          method: "POST",
          body: data,
          headers: HEADERS(rat.value, lang.value)
      })
      ...
      if (f.status === 201) {
          ...
          if (doall) router.push({name: "SecondaryWeapons"})
...
const doWeaponUnown = async (id: Number, doall: Boolean = false) => {
  ...
  // ===---
  try {
      let f = await fetch(`${API}/me/secondary-weapons/components/${id}/delete/`, {
          method: "DELETE",
          body: data,
          headers: HEADERS(rat.value, lang.value)
      })
