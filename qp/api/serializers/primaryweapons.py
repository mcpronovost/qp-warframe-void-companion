...
from qp.warframe.models import qpPrimaryWeapon, qpPrimaryWeaponComponent, qpPrimaryWeaponRelicReward

User = get_user_model()


class qpPrimaryWeaponSimpleSerializer(serializers.ModelSerializer):
    completion = serializers.SerializerMethodField()

    class Meta:
        model = qpPrimaryWeapon
        ...
        try:
            ...
            if request.user.is_authenticated:
                ...
                for component in obj.components.all():
                    c = request.user.primaryweapon_components.filter(
                        component=component
                    ).first()
                    if c is not None:
                        result += (100/total)
...
class qpPrimaryWeaponSerializer(qpPrimaryWeaponSimpleSerializer):
...
class qpPrimaryWeaponComponentSimpleSerializer(serializers.ModelSerializer):
...
class qpPrimaryWeaponRelicRewardSimpleSerializer(serializers.ModelSerializer):
    is_owned = serializers.SerializerMethodField()
    component = qpPrimaryWeaponComponentSimpleSerializer()
    ...
    def get_is_owned(self, obj):
        try:
            ...
            if ("slug" in kwargs and user is not None) or ("slug" not in kwargs and user.is_authenticated):
                result = user.primaryweapon_components.filter(
